...
def train(X_train, y_train, X_dev, y_dev, model,embedding, args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        loss_epoch = []
        g = gen_minibatch_HCL(X_train, y_train, args.batch_size,shuffle= True)
        for tokens1, tokens2, labels in g:
            ...
            logit = model(tokens1.cuda(), tokens2.cuda(),max_sents )
        ...
        model.eval()
        val_loss = []
        g = gen_minibatch_HCL(X_dev, y_dev, 4,shuffle= False)
        for tokens1, tokens2, labels in g:
            tokens1 = embedding(tokens1.long())
            tokens2 = embedding(tokens2.long())
            batch_size, max_sents, max_tokens, embed_size = tokens1.size()
...
def eval(X_train, y_train, model,embedding, args):
    ...
    p2 = []
    l = []
    g = gen_minibatch_HCL(X_train, y_train, 4 ,shuffle= False)
